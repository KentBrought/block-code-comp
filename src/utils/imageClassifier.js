...
const TFLITE_DIST_BASE =
  typeof window !== 'undefined'
    ? `${window.location.origin}${DEPLOY_BASE}/static/js/`
    : `${DEPLOY_BASE}/static/js/`
const TFLITE_BUNDLE_PATH =
  typeof window !== 'undefined'
    ? `${window.location.origin}${DEPLOY_BASE}/static/js/tf-tflite.es2017.js`
    : `${DEPLOY_BASE}/static/js/tf-tflite.es2017.js`
...
export function canvasHasDrawing(canvasElement, { minInkPixels = 24, threshold = 248 } = {}) {
  if (!canvasElement || !canvasElement.width || !canvasElement.height) return false
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (r < threshold || g < threshold || b < threshold) {
      inkPixels += 1
      if (inkPixels >= minInkPixels) return true
    }
  }
